...
if (!empty($_POST)) {
	...
	if(empty($error)) {
		...
		$_SESSION['join'] = $_POST;          //入力情報をセッションに代入//
...
if ($_REQUEST['action'] == 'rewrite' && isset($_SESSION['join'])) {    //URLパラメーター//
	$_POST = $_SESSION['join'];
}
?>
...
<form action="" method="post" enctype="multipart/form-data">
	<dl>
		...
		<dd>
			...
			<?php if($error['password'] === 'length' || $error['password'] ==='!empty') :?>
		...
		<dd>
			...
			<p class="error">*写真などは「.gif」または「.jpg」「png 」の
				画像を指定してください</p>
			<?php endif ; ?>
			<?php if (!empty($error)) :?>
